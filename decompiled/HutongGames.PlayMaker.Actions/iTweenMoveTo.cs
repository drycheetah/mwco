...
public class iTweenMoveTo : iTweenFsmAction
{
	...
	public override void OnDrawGizmos()
	{
		...
		for (int i = 0; i < transforms.Length; i++)
		{
			...
			{
				ref Vector3 reference = ref tempVct3[i];
				reference = ((!vectors[i].IsNone) ? vectors[i].Value : Vector3.zero);
			}
			else if (transforms[i].Value == null)
			{
				ref Vector3 reference2 = ref tempVct3[i];
				reference2 = ((!vectors[i].IsNone) ? vectors[i].Value : Vector3.zero);
			}
			else
			{
				ref Vector3 reference3 = ref tempVct3[i];
				reference3 = transforms[i].Value.transform.position + ((!vectors[i].IsNone) ? vectors[i].Value : Vector3.zero);
			}
		}
		...
		if (transforms.Length >= 2)
		{
			...
			if (!reverse.IsNone && reverse.Value)
			{
				for (int i = 0; i < transforms.Length; i++)
				{
					if (transforms[i].IsNone)
					{
						ref Vector3 reference = ref tempVct3[tempVct3.Length - 1 - i];
						reference = ((!vectors[i].IsNone) ? vectors[i].Value : Vector3.zero);
					}
					else if (transforms[i].Value == null)
					{
						ref Vector3 reference2 = ref tempVct3[tempVct3.Length - 1 - i];
						reference2 = ((!vectors[i].IsNone) ? vectors[i].Value : Vector3.zero);
					}
					else
					{
						ref Vector3 reference3 = ref tempVct3[tempVct3.Length - 1 - i];
						reference3 = ((space != Space.World) ? transforms[i].Value.transform.localPosition : transforms[i].Value.transform.position) + ((!vectors[i].IsNone) ? vectors[i].Value : Vector3.zero);
					}
				}
			}
			else
			{
				for (int j = 0; j < transforms.Length; j++)
				{
					...
					{
						ref Vector3 reference4 = ref tempVct3[j];
						reference4 = ((!vectors[j].IsNone) ? vectors[j].Value : Vector3.zero);
					}
					else if (transforms[j].Value == null)
					{
						ref Vector3 reference5 = ref tempVct3[j];
						reference5 = ((!vectors[j].IsNone) ? vectors[j].Value : Vector3.zero);
					}
					else
					{
						ref Vector3 reference6 = ref tempVct3[j];
						reference6 = ((space != Space.World) ? ownerDefaultTarget.transform.parent.InverseTransformPoint(transforms[j].Value.transform.position) : transforms[j].Value.transform.position) + ((!vectors[j].IsNone) ? vectors[j].Value : Vector3.zero);
					}
				}
			}
