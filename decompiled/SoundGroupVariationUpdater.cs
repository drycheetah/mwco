...
public class SoundGroupVariationUpdater : MonoBehaviour
{
	...
	private Transform Trans => GrpVariation.Trans;

	private AudioSource VarAudio => GrpVariation.VarAudio;

	private MasterAudioGroup ParentGroup => GrpVariation.ParentGroup;
	...
	private void MaybeChain()
	{
		...
		if (playSoundParm.AttachToSource || playSoundParm.SourceTrans != null)
		{
			...
			{
				MasterAudio.PlaySound3DFollowTransform(playSoundParm.SoundType, playSoundParm.SourceTrans, playSoundParm.VolumePercentage, playSoundParm.Pitch, delaySoundTime, null, isChaining: true);
			}
			else
			{
				MasterAudio.PlaySound3DAtTransform(playSoundParm.SoundType, playSoundParm.SourceTrans, playSoundParm.VolumePercentage, playSoundParm.Pitch, delaySoundTime, null, isChaining: true);
			}
		}
		else
		{
			MasterAudio.PlaySound(playSoundParm.SoundType, playSoundParm.VolumePercentage, playSoundParm.Pitch, delaySoundTime, null, isChaining: true);
		}
	}
